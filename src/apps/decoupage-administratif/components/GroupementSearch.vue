<template>
  <div class="groupement-search">
    <label class="fr-label" for="groupement-search-input">Rechercher un groupement</label>
    <div class="search-input-wrapper">
      <input
        id="groupement-search-input"
        v-model="query"
        type="search"
        class="fr-input"
    ...
    <ul v-if="results.length" class="search-results">
      ...
      >
        ...
        <span class="result-meta">SIREN <siren-link :value="result.code" /></span>
      </li>
    </ul>
    ...
    async search() {
      ...
      try {
        const response = await fetch(
          `${API_BASE}/groupement_collectivites_territoriales?nom=${encodeURIComponent(nom)}`
        );
        this.results = await response.json();
      } catch (error) {
        console.error('Erreur recherche groupements:', error);
        this.results = [];
      } finally {
        this.loading = false;
        this.searched = true;
