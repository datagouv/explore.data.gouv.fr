<template>
  <div class="aom-search">
    <label class="fr-label" for="aom-search-input">Rechercher une AOM</label>
    ...
    <ul v-if="results.length" class="search-results">
      ...
      >
        ...
        <span class="result-meta">
          SIREN <siren-link :value="result.code" />
          <span v-if="result.nbCommunes != null"> · {{ result.nbCommunes }} communes</span>
        </span>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    async search() {
      ...
      try {
        const response = await fetch(`${API_BASE}/aom?nom=${encodeURIComponent(nom)}`);
        this.results = await response.json();
      } catch (error) {
        console.error('Erreur recherche AOM:', error);
        this.results = [];
      } finally {
        this.loading = false;
        this.searched = true;
